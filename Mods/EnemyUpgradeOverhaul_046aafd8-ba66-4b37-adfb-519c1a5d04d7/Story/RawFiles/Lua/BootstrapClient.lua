...
local function LLENEMY_Client_InitializeModule()
	Ext.StatSetLevelScaling("Character", "Gain", LLENEMY_Ext_ScaleExperience)
	Ext.Print("[LLENEMY:BootstrapClient.lua:LLENEMY_ModuleLoading] Registered Gain scaling function.")
end

local function LLENEMY_Client_ModuleLoading()
	LLENEMY_Shared_InitModuleLoading()
	LLENEMY_Client_InitializeModule()
end
...
local function LLENEMY_Client_OnNetMessage(message, param)
	if message == "LLENEMY_SetExperienceScaling" then
		local enabled = param == "true"
		EnemyUpgradeOverhaul.ScaleExperience = enabled
		Ext.Print("[LLENEMY:BootstrapClient.lua:LLENEMY_Client_OnNetMessage] Set EnemyUpgradeOverhaul.ScaleExperience to (".. param ..").")
	end
end

Ext.RegisterNetListener("LLENEMY_SetExperienceScaling", LLENEMY_Client_OnNetMessage)